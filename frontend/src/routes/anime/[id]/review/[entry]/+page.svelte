...
<svelte:head>
  <title>Recenzie de {r.user.username} · {displayName(a)} · Anime-Kage</title>
</svelte:head>

<div class="container page">
  <a class="crumb" href={`/anime/${titleRef(a)}`}>← {displayName(a)}</a>

  <div class="layout">
    <aside class="side">
      <a class="poster media-tone" href={`/anime/${titleRef(a)}`}>
        {#if a.imageUrl}<img src={mediaUrl(a.imageUrl)} alt={displayName(a)} width="200" height="300" />{/if}
      </a>
    </aside>
    ...
    <main class="main">
      <header class="head">
        <div class="byline">
          ...
          <div class="byline-main">
            ...
            <h1 class="title">
              <a class="title-link" href={`/anime/${titleRef(a)}`}>{displayName(a)}</a>
              {#if a.year}<span class="year">{a.year}</span>{/if}
            </h1>
            <div class="meta">
              {#if starsOn}
      ...
      <div class="comments" id="comentarii">
        <CommentSection animeId={a.id} reviewId={r.entryId} flat heading="Comentarii" />
      </div>
    </main>
  </div>
</div>
