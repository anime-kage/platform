<script lang="ts">
  ...
  import { displayName, genreRo, type Manga, displaySynopsis} from '$lib/types';
  ...
  // Build a /manga URL preserving the other active params.
  ...
  const metaOf = (m: Manga) =>
    [m.year, (m.type ?? 'manga').toUpperCase(), m.chapters ? `${m.chapters} cap` : null]
      .filter(Boolean)
      .join(' · ');
</script>

<svelte:head><title>Manga · Anime-Kage</title></svelte:head>

<div class="container browse">
  <header class="top">
    <div>
      <p class="cat-kicker">Bibliotecă</p>
      <h1>Manga</h1>
    </div>
    ...
    <div class="controls">
      ...
      <div class="control">
        ...
        <select
          class="fsel"
          value={data.author}
          onchange={(e) => goto(hrefWith({ author: (e.currentTarget as HTMLSelectElement).value || null }))}
        >
          <option value="">Toți</option>
          {#each data.authors as au (au)}
  ...
  <p class="count">{data.count} titluri · subtitrare RO</p>

  {#if !data.list.length}
    <p class="empty">Niciun titlu în acest gen încă.</p>
  {:else if data.view === 'list'}
    <div class="listing">
      {#each data.list as m (m.id)}
        <a class="lrow" href={`/manga/${m.id}`}>
          <span class="lthumb" style={m.imageUrl ? `background-image:url(${mediaUrl(m.imageUrl)})` : ''}></span>
          <span class="lmain">
            <span class="lt">{displayName(m)}</span>
            <span class="lm">{metaOf(m)}</span>
          </span>
          <span class="ltags">
          ...
          {#if m.score}<span class="lscore">★ {m.score.toFixed(2)}</span>{/if}
        </a>
      {/each}
    </div>
  {:else}
    <div class="grid">
      {#each data.list as m (m.id)}
        <PosterCard a={m} href={`/manga/${m.id}`} />
      {/each}
    </div>
  {/if}
...
<style>
  ...
  /* featured banner — blurred backdrop + full contained poster
     (a raw 2:3 cover as background only ever shows its top edge) */
  .feat {
